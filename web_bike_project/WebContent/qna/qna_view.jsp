		...
		<div id="b_right">
			...
			<table class="boardForm">
				...
				<tbody>
					...
					<c:if test="${t_dto.getA_title() ne null}">
					<tr>
						<th>Reply</th>
						<td colspan="3"><textarea readonly cols="80" rows="20">${t_dto.getA_content()}</textarea></td>
					</tr>	
					<tr>
						<th>AnswerWriter</th>
						<td>${t_dto.getA_id()}</td>
			...
			<div class="buttonGroup">
				<c:if test="${sessionLevel eq 'member' && sessionId eq t_dto.getQ_id() && t_dto.getA_title() eq null}">
				...
				<c:if test="${sessionLevel eq 'admin' && t_dto.getA_title() eq null}">
				<a href="javascript:goReplyForm('${t_dto.getNo()}')" class="butt">Reply</a>
				</c:if>
				<c:if test="${sessionLevel eq 'admin' && t_dto.getA_title() ne null }">
