...
//	ArrayList<NoticeDto> dtos = dao.getNoticeList(select,search);
	ArrayList<PdsDto> dtos = dao.getPdsListPage(select,search,start,end);
...
<script type="text/javascript">
	...
	function goView(no){
		viewPds.t_no.value = no;
		viewPds.method="post";
		viewPds.action="pds_view.jsp";
		viewPds.submit();
	}
		...
		<table class="bord_table" summary="이표는 번호,제목,글쓴이,날자,조회수로 구성되어 있습니다">
			...
			<% for(int k=0; k<dtos.size(); k++){ %>
				<tr>
					...
					<td class="title"><a href="pds_view.jsp?t_no=<%=dtos.get(k).getNo()%>"><%=dtos.get(k).getTitle()%></a></td>
					<td>
